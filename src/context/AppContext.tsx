...
interface AppContextType {
  ...
  updateNote: (
    id: number,
    updates: Partial<Pick<Note, "title" | "content" | "category" | "pinned">>,
  ) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const signup = async (
    ...
    if (error) {
      ...
      if (
        ...
        return {
          error:
            "An account with this email already exists. Try Continue with Google or Sign in.",
        }
    ...
    if (data.user && !data.session) {
      const identities = (data.user as unknown as { identities?: unknown[] })
        ?.identities
  ...
  const signInWithOAuth = async (provider: "google"): Promise<{
    error?: string
  }> => {
  ...
  const verifyOtp = async (email: string, token: string): Promise<{
    error?: string
  }> => {
  ...
  const addTask = (t: Omit<Task, "id" | "createdAt">) => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("addTask sync:", error.message)
      })
  }
  ...
  const toggleTask = (id: number) => {
    ...
    supabase
      .from("tasks")
      .update({ completed: newVal, completed_at: newCompletedAt })
      .eq("id", id)

      .then(({ error }) => {
        if (error) console.error("toggleTask sync:", error.message)
      })
  }
  ...
  const deleteTask = (id: number) => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("deleteTask sync:", error.message)
      })
  }
  ...
  const addNote = (n: Omit<Note, "id" | "createdAt" | "updatedAt">) => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("addNote sync:", error.message)
      })
  }
  ...
  const updateNote = (
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("updateNote sync:", error.message)
      })
  }
  ...
  const deleteNote = (id: number) => {
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("deleteNote sync:", error.message)
      })
  }

  const createLinked = (
    taskBase: Omit<Task, "id" | "createdAt" | "linkedNoteId">,
    noteTitle: string,
  ) => {
    ...
    const newNote: Note = {
      ...
      category: (taskBase.category === "health"
        ? "personal"
        : taskBase.category) as NoteCategory,
    ...
    const newTask: Task = {
      ...
      id: taskId,
      createdAt: now,
      completedAt: taskBase.completed ? now : null,
      linkedNoteId: noteId,
    }
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("createLinked note sync:", error.message)
      })
    ...
    supabase
      ...
      .insert({
        ...
        user_id: user.id,
        text: taskBase.text,
        completed: taskBase.completed,

        priority: taskBase.priority,
        category: taskBase.category,
        time: taskBase.time ?? null,

        linked_note_id: noteId,
        created_at: now,
        completed_at: taskBase.completed ? now : null,
      })
      .then(({ error }) => {
        if (error) console.error("createLinked task sync:", error.message)
      })
  ...
  const linkTaskToNote = (taskId: number, noteId: number) => {
    setTasks((prev) =>
      prev.map((t) => (t.id === taskId ? { ...t, linkedNoteId: noteId } : t)),
    )

    setNotes((prev) =>
      prev.map((n) => (n.id === noteId ? { ...n, linkedTaskId: taskId } : n)),
    )
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("linkTaskToNote task sync:", error.message)
      })
    ...
    supabase
      ...
      .then(({ error }) => {
        if (error) console.error("linkTaskToNote note sync:", error.message)
      })
  }
